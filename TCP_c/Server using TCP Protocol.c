...

{
        ...
        if( (tfd = t_open(DEV_TCP, OTDWR, (struct t_info *) 0)) < 0) 
            err_dump(:server: cant of %s", DEV_TCP); 
         ...
         serv_addr.sin_addr.s_addr = htonl(INADDRPANY);
         ...
         if(t_bind(tfd, &req, (struct t_bind *) 0) < 0) 
            err_dump ("Server: cant bind to local address"); 
            
         
         //Allocate a t_call structrue for t_listen() and t_accept(). 
         
         if ( (callptr = (struct t_call *) t_alloc(tfd, T_CALL, T_ADDR)) == NULL) 
          err_drump("server: t_alloc error for T_CALL"); 
          
        for(;;) { 
        
        
            //wait for a connection from a client proces. 
